...
export default class AssetManager {
	...
	async preload(atlas: atlassheet[]): Promise<void> {
		this.promises.push(
			new Promise<void>((resolve, _reject) => {
				...
				this.assetsloader.load((_assetsloader, _resources) => {
					...
					// Добавил вот тут вывод в консоль для всех проблемных ресурсов.
					// Внимание! Сейчас я обрабатываю ЛЮБУЮ ошибку как ошибку ненахождения ресурса.
					// вероятно, правильнее было бы смотреть на тип ошибки и выводить соответствующее сообщение.
		...
		await Promise.all(this.promises)
			.then((_result) => {
				this.Element?.classList.add("preloader--hide");
				console.warn("preloader hide");
			})
